/*
We're constantly improving the code you see. 
Please share your feedback here: https://form.asana.com/?k=uvp-HPgd3_hyoXRBw1IcNg&d=1152665201300829
*/

import React from "react";

interface Props {
  className: any;
}

export const PlatformTelegramColorOriginal = ({ className }: Props): JSX.Element => {
  return (
    <svg
      className={`${className}`}
      fill="none"
      height="48"
      viewBox="0 0 48 48"
      width="48"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clipPath="url(#clip0_12_125)">
        <path
          d="M24 48C37.2548 48 48 37.2548 48 24C48 10.7452 37.2548 0 24 0C10.7452 0 0 10.7452 0 24C0 37.2548 10.7452 48 24 48Z"
          fill="url(#paint0_linear_12_125)"
        />
        <path
          clipRule="evenodd"
          d="M10.8638 23.7466C17.8603 20.6984 22.5257 18.6888 24.86 17.7179C31.5251 14.9456 32.91 14.4641 33.8127 14.4482C34.0112 14.4447 34.4552 14.4939 34.7427 14.7272C34.9855 14.9242 35.0523 15.1904 35.0843 15.3772C35.1163 15.5639 35.1561 15.9895 35.1244 16.3219C34.7632 20.1169 33.2004 29.3263 32.4053 33.5767C32.0689 35.3752 31.4065 35.9783 30.7652 36.0373C29.3714 36.1655 28.3131 35.1162 26.9632 34.2313C24.8509 32.8467 23.6576 31.9847 21.6072 30.6336C19.2377 29.0721 20.7737 28.2139 22.1241 26.8113C22.4775 26.4442 28.6183 20.8587 28.7372 20.352C28.752 20.2886 28.7658 20.0524 28.6255 19.9277C28.4852 19.803 28.2781 19.8456 28.1286 19.8795C27.9167 19.9276 24.5423 22.158 18.0053 26.5707C17.0474 27.2284 16.1799 27.5489 15.4026 27.5321C14.5456 27.5136 12.8973 27.0475 11.6719 26.6492C10.1688 26.1607 8.97429 25.9023 9.07831 25.0726C9.13249 24.6404 9.72764 24.1984 10.8638 23.7466Z"
          fill="white"
          fillRule="evenodd"
        />
      </g>
      <defs>
        <linearGradient gradientUnits="userSpaceOnUse" id="paint0_linear_12_125" x1="24" x2="24" y1="0" y2="47.644">
          <stop stopColor="#2AABEE" />
          <stop offset="1" stopColor="#229ED9" />
        </linearGradient>
        <clipPath id="clip0_12_125">
          <rect fill="white" height="48" width="48" />
        </clipPath>
      </defs>
    </svg>
  );
};
